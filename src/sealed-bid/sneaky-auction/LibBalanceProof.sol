...
library LibBalanceProof {
    ...
    function _processLeafNode(
        ...
        accountBalance = _decodeAccountBalance(
            accountStatePtr,
            accountStateLen
        );
    }
    ...
    function _processExtensionNode(
        ...
        uint256 encodedPathPtr;
        uint256 encodedPathLen;
        (encodedPathPtr, encodedPathLen, childHash) = _decodeExtensionNode(
            nodeRLP
        );
        ...
        if (
            flag != EXTENSION_NODE_EVEN_LENGTH &&
            flag != EXTENSION_NODE_ODD_LENGTH
        ) {
    ...
    function _rlpItemLength(
        uint256 memPtr
    ) private pure returns (uint256 itemLen) {
        uint256 byte0;
        assembly {
    ...
    function _getNibble(
        bytes32 path,
        uint256 pathBitIndex
    ) private pure returns (uint256 nibble) {
